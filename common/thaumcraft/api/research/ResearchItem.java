...
public class ResearchItem
{
    ...
    /**
     * A longer description of the research. This is the text shown in the
     * handheld research scroll and the research table.
     */
    ...
    /**
     * the itemstack of an item or block that will be used as the icon for this
     * research
     */
    public final ItemStack itemStack;

    /**
     * the index within the research.png file used for this research if it does
     * not use an item icon
     */
     ...
     * used for research that doesn't have recipes or that unlocks automatically
     * via the sibling system
     */
    private boolean isStub;
    /**
     * Indicates research that cannot be gained by normal means (either via
     * normal or lost research), but still uses a normal icon. Works much like
     * isStub but is handy for mods that wish to add research through their own
     * means and keep a normal icon.
     */
    ...
    public ResearchItem(String par1, ObjectTags tags, int par3, int par4,
            int icon)
    {
    ...
    public ResearchItem(String par1, ObjectTags tags, int par3, int par4,
            ItemStack par5ItemStack, int icon)
    {
        key = par1;
        this.tags = tags;
        name = "";
        ...
        if (el != null)
        {
            ...
            for (int a = 0; a < children.getLength(); a++)
            {
                ...
                {
                    name = children.item(a).getTextContent();
                } else if (children.item(a).getNodeName().equals("longText"))
                {
                    longText = children.item(a).getTextContent();
                } else if (children.item(a).getNodeName().equals("popupText"))
                {
                    popupText = children.item(a).getTextContent();
                }
            }
        }
