...
public class InventoryDiving implements IInventory {
    ...
    public void readFromNBT(NBTTagCompound nbtTagCompound) {
        NBTTagCompound contentTag = ((NBTTagCompound) nbtTagCompound
                .getTag("divingGearInv"));
        if (contentTag == null)
            return;
    ...
    public boolean isItemValidForSlot(int i, ItemStack itemStack) {
        if (inventory[i] == null)
            ...
            if (inventory[i].getItem().equals(itemStack.getItem())) {
                if ((inventory[i].stackSize + itemStack.stackSize) <= inventory[i]
                        .getMaxStackSize())
                    return true;
                else
                    return false;
