...
public class ItemAdvGear extends ItemArmor implements IGear{
	...
	public ItemAdvGear(int par1, EnumArmorMaterial par2EnumArmorMaterial,
			int par3, int par4, String textureItem, int inventorySize, int item, int type) {
	...
	public void onUpdate(ItemStack itemStack, World world, Entity entity, 
		...
		if(ContainerDiving.class.isAssignableFrom(((EntityPlayer)entity).openContainer.getClass())){
			container = (ContainerDiving)((EntityPlayer)entity).openContainer;
			if(container.updateNotification){
				container.helmInventory.writeToNBT(itemStack.stackTagCompound, NBTUtil.getString(itemStack, "uniqueID"));
    ...
    public void registerIcons(IconRegister iconRegister){
		this.itemIcon = iconRegister.registerIcon(ModStrings.MOD_ID.toLowerCase() + ":" + this.texture);
	}
	...
	private void useAir(ItemStack stack, EntityPlayer player){
		for(int i = 0; i < this.container.helmInventory.getSizeInventory(); i++){
			if(this.container.helmInventory.getStackInSlot(i) != null &&
				(this.container.helmInventory.getStackInSlot(i).getItem() instanceof IAirTanks)){
				int usesLeft = (this.container.helmInventory.getStackInSlot(i).getMaxDamage() - 
					this.container.helmInventory.getStackInSlot(i).getItemDamage());
				if(this.container.helmInventory.getStackInSlot(i).getItemDamage() < 
					this.container.helmInventory.getStackInSlot(i).getMaxDamage()){
					player.setAir(300);
					player.addChatMessage("["+stack.getDisplayName()+"]: " + usesLeft + "Uses left.");
					this.container.helmInventory.getStackInSlot(i).damageItem(1, player);
					this.container.helmInventory.writeToNBT(stack.stackTagCompound, NBTUtil.getString(stack, "uniqueID"));
					break;
				}
			}
		}
