...
public class TileEntityWaterMill extends TileEntity implements IPowerEmitter,
        IPowerReceptor {
    ...
    public boolean isActive() {
        int j = 0;
        if (BlockUtil.isBlockAdjacentToWaterSource(worldObj, xCoord, yCoord,
                zCoord)) {
    ...
    public void updateEntity() {
        ...
        if (!this.worldObj.isRemote) {
            if (isActive()) {
                ...
                tick++;
                if (tick >= (ModInts.ticks / 4))
                    sendPower();
                if (tick > ModInts.ticks) {
                    this.powerHandler.addEnergy(0.5F);
                    tick = 0;
                }
            } else {
                tick = 0;
    ...
    public void sendPower() {
        ...
        if (tile != null && tile instanceof IPowerReceptor) {
            ...
            if (receptor != null) {
                ...
                if (extracted > 0F
                        && this.powerHandler.getEnergyStored() >= extracted) {
                    this.powerHandler.useEnergy(extracted, extracted, false);
                    receptor.receiveEnergy(Type.ENGINE, extracted,
                            ForgeDirection.DOWN);
                }
            }
    ...
    public float getPowerToExtract(PowerReceiver receptor) {
        return ((receptor.getMinEnergyReceived() + receptor
                .getMaxEnergyReceived()) / 15F);
    }
